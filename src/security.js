...
export const DEV_WHITELIST = ['thejxxuu@gmail.com', 'jeevanh259@gmail.com', 'testuser@gmail.com'];
...
export const checkRateLimit = (userId, type = 'report') => {
  ...
  if (!Object.prototype.hasOwnProperty.call(COOLDOWNS, type)) {
    return { ok: true };
  }

  const userData = submissionHistory.get(userId) || new Map();
  const lastTime = userData.get(type) || 0;

  if (now - lastTime < COOLDOWNS[type]) {
    return { ok: false, remaining: Math.ceil((COOLDOWNS[type] - (now - lastTime)) / 1000) };
  }

  userData.set(type, now);
